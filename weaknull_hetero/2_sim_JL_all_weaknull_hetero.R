...
load("data_0.3_0.4_weaknull_hetero/setup.RData")
...
load(str_glue("data_0.3_0.4_weaknull_hetero/simdata_main_new_{main_case}.RData"))
load(str_glue("data_0.3_0.4_weaknull_hetero/simdata_new_{case}.RData"))
# load(str_glue("data/simdata_main_new_{main_case}.RData"))
# load(str_glue("data/simdata_new_{case}.RData"))

# 2 simulation -------------------------------------------------

if (!dir.exists("output_weaknull_hetero")) {dir.create("output_weaknull_hetero")}
if (!dir.exists("output_weaknull_hetero/cache")) {dir.create("output_weaknull_hetero/cache")}
writeLines("", str_glue("output_weaknull_hetero/cache/prog_{case}.txt"))
...
raw_result <- mclapply(1:n_rep, function(i_rep) {
  ...
  if (i_rep == 1) {
    save(
      ...
      file = str_glue("output_weaknull_hetero/cache/dt_{case}_{i_rep}.RData")
    )
    o_f <- T
  } else {
    o_f <- F
  ...
  if (ep == "binary") {
    tryCatch({
      ...
      save(
        ...
        file = str_glue("output_weaknull_hetero/cache/err_{case}_{i_rep}.RData")
      )
    })
  }

  # output
  cat(i_rep, "\n", file = str_glue("output_weaknull_hetero/cache/prog_{case}.txt"), 
      append = TRUE)
...
save(raw_result, count_null, total_time, file = str_glue("output_weaknull_hetero/cache/raw_{case}.RData"))
# save(raw_result, total_time, file = str_glue("output_weaknull_hetero/cache/raw_{case}.RData"))
...
save(inf_result, file = str_glue("output_weaknull_hetero/cache/inf_{case}.RData"))
...
save(metrics, file = str_glue("output_weaknull_hetero/metrics_{case}.RData"))
