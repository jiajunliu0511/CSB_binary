...
if (!dir.exists(str_glue("data_{prob$p0}_{prob$p1}_weaknull_hetero"))) {dir.create(str_glue("data_{prob$p0}_{prob$p1}_weaknull_hetero"))}
...
for (case in 1:nrow(setup)) {
  ...
  if (mag_bias == 0) {
    ...
    save(simdata_main, file = str_glue("data_{prob$p0}_{prob$p1}_weaknull_hetero/simdata_main_new_{case}.RData"))
    
  } else {
    # load(str_glue("data/simdata_main_new_{main_case}.RData"))
    load(str_glue("data_{prob$p0}_{prob$p1}_weaknull_hetero/simdata_main_new_{case}.RData"))
  }
  ...
  simdata <- mclapply(1:n_rep, function(i_rep) {
    ...
    if (om) {
      X_mu <- simdata_main[[i_rep]]$X
    } else {
      X_mu <- simdata_main[[i_rep]]$Xt
    }
    ...
    # heterogeneity
    gamma_het <- c(0.5, -0.5, 0.5)
    
    # (Y, Ynull) under weak null
    if (ep == "binary") {
      mu1_het_raw <- mu0 + as.vector(X_mu %*% gamma_het)
      # mu1_het_intercept <- uniroot(function(.x) {
      #   mean(plogis(.x + mu1_het_raw)) - mean(plogis(mu0))
      # }, interval = c(-100, 100))$root
      mu1_het_intercept <- uniroot(function(.x) {
        mean(plogis(.x + mu1_het_raw)[S == 1]) -
          mean(plogis(mu0)[S == 1])
      }, interval = c(-100, 100))$root
      mu1_weaknull <- mu1_het_intercept + mu1_het_raw
      Y1_weaknull <- rbinom(N, size = 1, prob = plogis(mu1_weaknull))
    } else if (ep == "continuous") {
      shift <- as.vector(X_mu %*% gamma_het) - mean(as.vector(X_mu %*% gamma_het))
      mu1_weaknull <- mu0 + shift
      Y1_weaknull <- mu1_weaknull + rnorm(N)
    }
    ## for power (alternative, unchanged)
    Y <- S * A * Y1 + S * (1 - A) * Y0 + (1 - S) * Y00
    ## for type I error (weak null)
    Ynull <- S * A * Y1_weaknull + S * (1 - A) * Y0 + (1 - S) * Y00
    
    ## diagnostic check
    p0_rct <- plogis(mu0)[S == 1]
    p1_rct <- plogis(mu1_weaknull)[S == 1]
    
    check_weaknull <- tibble(
      mean_RD_null = mean(p1_rct - p0_rct),
      sd_CATE_null = sd(p1_rct - p0_rct),
      min_CATE_null = min(p1_rct - p0_rct),
      max_CATE_null = max(p1_rct - p0_rct)
    )
    # output
    lst(mu00, Y00, Y, Ynull, id_unbias,check_weaknull)
  }, mc.cores = n_cores)
  save(simdata, file = str_glue("data_{prob$p0}_{prob$p1}_weaknull_hetero/simdata_new_{case}.RData"))
}

save(setup, file = str_glue("data_{prob$p0}_{prob$p1}_weaknull_hetero/setup.RData"))
