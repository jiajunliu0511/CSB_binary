...
if (!dir.exists(str_glue("data_{prob$p0}_{prob$p1}"))) {dir.create(str_glue("data_{prob$p0}_{prob$p1}"))}
...
for (case in 1:nrow(setup)) {
  ...
  if (mag_bias == 0) {
    ...
    save(simdata_main, file = str_glue("data_{prob$p0}_{prob$p1}/simdata_main_new_{case}.RData"))
    
  } else {
    # load(str_glue("data/simdata_main_new_{main_case}.RData"))
    load(str_glue("data_{prob$p0}_{prob$p1}/simdata_main_new_{case}.RData"))
  }
  ...
  save(simdata, file = str_glue("data_{prob$p0}_{prob$p1}/simdata_new_{case}.RData"))
}

save(setup, file = str_glue("data_{prob$p0}_{prob$p1}/setup.RData"))
